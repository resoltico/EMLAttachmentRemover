...
def test_release_artifact_names_are_v3_and_include_all_public_distribution_forms() -> (
    ...
    names = (
        "eml_attachment_remover-3.0.3-cp314-none-any.whl",
        "eml_attachment_remover-3.0.3.tar.gz",
...
def test_release_tag_script_entrypoint_uses_the_same_version_contract(
    ...
    monkeypatch.setattr(sys, "argv", [str(RELEASE_TAG_TOOL), "v3.0.3"])
...
def test_v3_release_notes_describe_the_corrected_mime_pruned_boundary() -> None:
    notes = PROJECT_ROOT / ".github" / "release-notes" / "v3.0.3.md"
    content = notes.read_text(encoding="utf-8")
    assert content.startswith("# EML Attachment Remover 3.0.3\n")
